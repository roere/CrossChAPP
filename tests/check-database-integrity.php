...
$settings=new MailSettingsRepository($db);$mail=new MailService($settings,static function():void{});$service=new InvitationService(new InvitationRepository($db),$users,$settings,$mail);$admin=$users->findByLogin('admin');$service->invite(['first_name'=>'Ina','last_name'=>'Invite','email'=>'invite@example.test','home_chapter_org_id'=>1],(int)$admin['id'],true);$duplicate=false;try{$service->invite(['first_name'=>'Ina','last_name'=>'Invite','email'=>'INVITE@example.test','home_chapter_org_id'=>1],(int)$admin['id'],true);}catch(DomainException){$duplicate=true;}$ok($duplicate,'Doppelte aktive Einladung wird fachlich verhindert.');
