...
$manager=$users->create('Manager','Konto','manager-account@example.test',password_hash('manager-password',PASSWORD_DEFAULT),99);$managerId=(int)$manager['id'];$pdo->exec("UPDATE users SET role='user_manager',status='active',email_verified_at=CURRENT_TIMESTAMP WHERE id=$managerId");$managerUnchanged=$service->updateHomeChapter($managerId,99,false);$check($managerUnchanged['result']==='unchanged'&&(int)$managerUnchanged['account']['homeChapterOrgId']===99,'Anwenderbetreuer darf den normalen unveränderten Account-Updatepfad verwenden.');$managerRemoved=$service->updateHomeChapter($managerId,null,false);$check($managerRemoved['result']==='removed'&&$managerRemoved['account']['homeChapterOrgId']===null,'Anwenderbetreuer darf sein eigenes Heimatchapter wie ein normaler Anwender bearbeiten.');$adminUpdateBlocked=false;try{$service->updateHomeChapter((int)$admin['id'],null,false);}catch(DomainException$exception){$adminUpdateBlocked=$exception->getMessage()==='Administratorkonten können hier nicht bearbeitet werden.';}$check($adminUpdateBlocked,'Der Volladmin-Schutz des Account-Updatepfads bleibt aktiv.');
...
$details=$service->account($deleteId);$check($details['firstName']==='Lösch'&&$details['lastName']==='Test'&&$details['email']==='delete@example.test'&&$details['verificationStatus']==='unverified'&&$details['canDelete']&&!array_key_exists('id',$details),'Kontodaten enthalten Status und nur erlaubte Felder.');
