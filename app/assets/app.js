(() => {
    ...
    const state = {
        ...
        batchRunning: false, batchStopRequested: false, users: [], selectedAdminUserId: null, pendingRoleChange: null,representations:[],
        textTemplates: [], selectedTextTemplateKey: null, textTemplateDirty: false, pendingTextTemplateKey: null, pendingInvitationOverride: null, pendingInvitationResend: null, userErrors:[], performanceLoading:false,performanceTimer:null,performanceInitialized:false,performanceTimerGeneration:0,
    };
    ...
    const elements = {
        ...
        reportsPanel: document.querySelector('#reports-panel'), userErrorTableWrap:document.querySelector('#user-error-table-wrap'),userErrorList:document.querySelector('#user-error-list'),userErrorEmpty:document.querySelector('#user-error-empty'),userErrorMessage:document.querySelector('#user-error-message'),ambiguousDialog:document.querySelector('#ambiguous-diagnostic-dialog'),ambiguousDetails:document.querySelector('#ambiguous-diagnostic-details'),ambiguousLinks:document.querySelector('#ambiguous-diagnostic-links'),closeAmbiguous:document.querySelector('#close-ambiguous-diagnostic'),representationPanel:document.querySelector('#representation-history-panel'),representationTitle:document.querySelector('#representation-history-title'),representationList:document.querySelector('#representation-history-list'),representationEmpty:document.querySelector('#representation-history-empty'),representationMessage:document.querySelector('#representation-history-message'), performanceWindow: document.querySelector('#bni-performance-window'),performanceRefresh:document.querySelector('#bni-performance-refresh'), performanceStats: document.querySelector('#bni-performance-stats'), performanceChart: document.querySelector('#bni-performance-chart'), performanceLine: document.querySelector('#bni-performance-line'), performanceYMax: document.querySelector('#bni-performance-y-max'), performanceStart: document.querySelector('#bni-performance-start'), performanceEnd: document.querySelector('#bni-performance-end'), performanceMessage: document.querySelector('#bni-performance-message'),throttleStatus:document.querySelector('#bni-throttle-status'),throttleCurrent:document.querySelector('#bni-throttle-current'),throttleStats:document.querySelector('#bni-throttle-stats'),throttleChart:document.querySelector('#bni-throttle-chart'),throttleAverageLine:document.querySelector('#bni-throttle-average-line'),throttleMaximumLine:document.querySelector('#bni-throttle-maximum-line'),throttleYMax:document.querySelector('#bni-throttle-y-max'),throttleStart:document.querySelector('#bni-throttle-start'),throttleEnd:document.querySelector('#bni-throttle-end'),
    ...
    const representationSortState=FULL_ADMIN?window.CrossChappSort.bind(document.querySelector('#representation-history-table'),renderRepresentationHistory):{key:'date',direction:'descending'};
    const sortFields = {
    ...
    const representationSortFields={type:{type:'string',value:item=>item.type},person:{type:'string',value:item=>item.person},chapter:{type:'string',value:item=>item.chapter},secondPerson:{type:'string',value:item=>item.secondPerson||'—'},secondChapter:{type:'string',value:item=>item.secondChapter||'—'},date:{type:'date',value:item=>item.date},occurred:{type:'number',value:item=>item.occurred?1:0}};

    if(FULL_ADMIN){
        const chapterData=document.querySelector('#chapter-data-panel'),chapterContent=chapterData?.querySelector('.chapter-data-content'),chapterControls=chapterContent?.querySelector('.controls'),chapterList=chapterContent?.querySelector('#chapter-list'),batchPanel=chapterContent?.querySelector('.batch-panel');
        if(chapterContent&&chapterControls&&chapterList&&batchPanel){chapterContent.insertBefore(chapterControls,batchPanel);chapterContent.insertBefore(chapterList,batchPanel);}
        if(chapterData&&elements.automationPanel)chapterData.after(elements.automationPanel);
        representationSortState.key='date';representationSortState.direction='descending';document.querySelector('#representation-history-table th[data-sort-key="date"]')?.setAttribute('aria-sort','descending');document.querySelector('#representation-history-table th[data-sort-key="date"] .sort-indicator').textContent='▼';elements.representationPanel.addEventListener('toggle',()=>{if(elements.representationPanel.open&&!elements.representationPanel.dataset.loaded)loadRepresentationHistory();});
    ...
    async function loadRepresentationHistory(){elements.representationMessage.textContent='Vertretungshistorie wird geladen …';try{const response=await fetch('/api/admin/representations.php'),payload=await response.json();if(!response.ok)throw new Error(payload.error||'Die Vertretungshistorie konnte nicht geladen werden.');state.representations=Array.isArray(payload.items)?payload.items:[];elements.representationTitle.textContent=`Vertretungen (${payload.total??state.representations.length})`;elements.representationPanel.dataset.loaded='true';renderRepresentationHistory();elements.representationMessage.textContent=payload.total>payload.limit?`Es werden die neuesten ${payload.limit} von ${payload.total} Einträgen angezeigt.`:'';}catch(error){elements.representationMessage.textContent=error.message;elements.representationMessage.className='message error';}}
    function renderRepresentationHistory(){if(!elements.representationList)return;const items=window.CrossChappSort.sort(state.representations,representationSortState,representationSortFields),format=new Intl.DateTimeFormat('de-DE',{timeZone:'Europe/Berlin',weekday:'short',day:'2-digit',month:'2-digit',year:'numeric'});elements.representationList.replaceChildren(...items.map(item=>{const row=document.createElement('tr');row.dataset.rowKey=item.rowKey;if(item.assignmentStatus==='cancelled')row.title='Vereinbarung storniert';for(const value of[item.type,item.person,item.chapter,item.secondPerson||'—',item.secondChapter||'—',format.format(new Date(`${item.date}T12:00:00Z`))]){const cell=document.createElement('td');cell.textContent=value;row.append(cell);}const occurred=document.createElement('td');if(item.occurred){const mark=document.createElement('span');mark.className='representation-occurred';mark.textContent='✓';mark.setAttribute('aria-label','Vertretung stattgefunden');occurred.append(mark);}else{occurred.textContent='—';if(item.assignmentStatus==='cancelled')occurred.title='Vereinbarung storniert';}row.append(occurred);return row;}));elements.representationEmpty.hidden=items.length!==0;}

    async function loadBniPerformance(){if(state.performanceLoading)return false;state.performanceLoading=true;const windowMinutes=Number(elements.performanceWindow.value);elements.performanceWindow.disabled=true;elements.performanceMessage.textContent='Leistungsdaten werden geladen …';try{const response=await fetch(`/api/admin/bni-performance.php?window_minutes=${windowMinutes}`),payload=await response.json();if(!response.ok)throw new Error(payload.error||'Leistungsdaten konnten nicht geladen werden.');renderBniPerformance(payload);elements.performanceMessage.textContent='';return true;}catch(error){elements.performanceMessage.textContent=error.message;elements.performanceMessage.className='message error';return false;}finally{state.performanceLoading=false;elements.performanceWindow.disabled=false;}}
    function initializePerformanceRefresh(){const allowed=['1','2','5','10','60'],stored=localStorage.getItem('crosschapp_bni_performance_refresh_minutes');elements.performanceRefresh.value=allowed.includes(stored)?stored:'5';state.performanceInitialized=true;document.addEventListener('visibilitychange',()=>{if(!state.performanceInitialized)return;if(document.hidden){stopPerformanceTimer();return;}if(elements.reportsPanel.open)loadBniPerformance().finally(startPerformanceTimer);});window.addEventListener('pagehide',()=>{state.performanceInitialized=false;stopPerformanceTimer();},{once:true});}
    function stopPerformanceTimer(){if(state.performanceTimer!==null){clearTimeout(state.performanceTimer);state.performanceTimer=null;}elements.performanceRefresh.dataset.timerActive='false';}
    function startPerformanceTimer(){stopPerformanceTimer();if(!state.performanceInitialized||!elements.reportsPanel.open||document.hidden)return;state.performanceTimerGeneration++;elements.performanceRefresh.dataset.timerActive='true';elements.performanceRefresh.dataset.timerGeneration=String(state.performanceTimerGeneration);state.performanceTimer=setTimeout(async()=>{state.performanceTimer=null;elements.performanceRefresh.dataset.timerActive='false';if(!document.hidden)await loadBniPerformance();startPerformanceTimer();},Number(elements.performanceRefresh.value)*60000);}
    function changePerformanceRefresh(){if(!['1','2','5','10','60'].includes(elements.performanceRefresh.value))elements.performanceRefresh.value='5';localStorage.setItem('crosschapp_bni_performance_refresh_minutes',elements.performanceRefresh.value);startPerformanceTimer();}
    async function loadUserErrors(){elements.userErrorMessage.textContent='Meldungen werden geladen …';try{const response=await fetch('/api/admin/messages.php?limit=100'),payload=await response.json();if(!response.ok)throw new Error(payload.error||'Meldungen konnten nicht geladen werden.');const messages=Array.isArray(payload.messages)?payload.messages:[];state.userErrors=messages;elements.userErrorList.replaceChildren(...messages.map(item=>{const row=document.createElement('tr'),time=document.createElement('td'),user=document.createElement('td'),technical=document.createElement('td'),meta=document.createElement('td');time.textContent=new Intl.DateTimeFormat('de-DE',{timeZone:'Europe/Berlin',dateStyle:'short',timeStyle:'medium'}).format(new Date(item.created_at));user.textContent=item.user_message;technical.textContent=item.technical_message;meta.textContent=[item.error_code,item.context].filter(Boolean).join(' / ')||'—';if(item.error_code==='ambiguous'){const button=document.createElement('button');button.type='button';button.className='secondary compact-button';button.dataset.ambiguousId=String(item.id);button.textContent='Chapter prüfen';meta.append(document.createElement('br'),button);}row.append(time,user,technical,meta);return row;}));elements.userErrorTableWrap.hidden=messages.length===0;elements.userErrorEmpty.hidden=messages.length!==0;elements.userErrorMessage.textContent='';}catch(error){elements.userErrorMessage.textContent=error.message;elements.userErrorMessage.className='message error';}}
    function openAmbiguousDiagnostic(event){const button=event.target.closest('[data-ambiguous-id]');if(!button)return;const item=state.userErrors.find(entry=>String(entry.id)===button.dataset.ambiguousId);if(!item)return;const fields=[['Geprüfter Name',[item.checked_first_name,item.checked_last_name].filter(Boolean).join(' ')||'—'],['Chapter',item.chapter_name||'—'],['org_id',item.org_id??'—'],['Treffer',item.match_count??0],['Kontext',item.context||'—'],['Technische Ursache',item.technical_message||'—']];elements.ambiguousDetails.replaceChildren(...fields.flatMap(([label,value])=>{const dt=document.createElement('dt'),dd=document.createElement('dd');dt.textContent=label;dd.textContent=String(value);return[dt,dd];}));const links=(Array.isArray(item.matches)?item.matches:[]).filter(match=>match&&match.profileUrl);elements.ambiguousLinks.replaceChildren();if(links.length){const heading=document.createElement('h3');heading.textContent='Gefundene BNI-Einträge';elements.ambiguousLinks.append(heading,...links.map((match,index)=>{const link=document.createElement('a');link.href=match.profileUrl;link.target='_blank';link.rel='noopener noreferrer';link.textContent=`Treffer ${index+1} ↗`;return link;}));}else{const hint=document.createElement('p');hint.textContent='Für die Treffer liegt kein sicher navigierbarer Profillink vor.';elements.ambiguousLinks.append(hint);}elements.ambiguousDialog.showModal();}
    function renderBniPerformance(payload){const boxes=(target,fields)=>target.replaceChildren(...fields.map(([label,value])=>{const box=document.createElement('div'),strong=document.createElement('strong'),span=document.createElement('span');strong.className='stat-value';span.className='stat-label';strong.textContent=value;span.textContent=label;box.append(strong,document.createTextNode(' '),span);return box;})),peak=payload.peak_30d||{},dateTime=value=>value?new Intl.DateTimeFormat('de-DE',{timeZone:'Europe/Berlin',dateStyle:'medium',timeStyle:'medium'}).format(new Date(value)):'—',peakTime=peak.minute?new Intl.DateTimeFormat('de-DE',{timeZone:'Europe/Berlin',dateStyle:'medium',timeStyle:'short'}).format(new Date(peak.minute)):'—';boxes(elements.performanceStats,[['Requests letzte 24 Stunden',payload.requests_24h||0],['Requests letzte 60 Minuten',payload.requests_60m||0],['Höchste Anfragenlast innerhalb einer Minute',`${peak.count||0} Anfragen / Minute`],['Peak letzte 30 Tage',peak.minute?`am ${peakTime} Uhr`:'Kein Request']]);const points=Array.isArray(payload.last_24h)?payload.last_24h:[],maximum=Math.max(1,...points.map(point=>Number(point.count)||0)),width=666,height=178;elements.performanceLine.setAttribute('points',points.map((point,index)=>`${42+(points.length<2?0:index/(points.length-1)*width)},${190-(Number(point.count)||0)/maximum*height}`).join(' '));elements.performanceYMax.textContent=String(maximum);const timeFormat=new Intl.DateTimeFormat('de-DE',{timeZone:'Europe/Berlin',hour:'2-digit',minute:'2-digit'});elements.performanceStart.textContent=points.length?timeFormat.format(new Date(points[0].minute)):'--:--';elements.performanceEnd.textContent=points.length?timeFormat.format(new Date(points[points.length-1].minute)):'--:--';elements.performanceChart.querySelector('desc').textContent=`Rollierende Anzahl der BNI-Anfragen in jeweils ${payload.window_minutes} Minuten. Maximum ${maximum}.`;const throttle=payload.throttle||{},seconds=value=>value===null||value===undefined?'—':`${(Number(value)/1000).toLocaleString('de-DE',{minimumFractionDigits:1,maximumFractionDigits:2})} s`;elements.throttleStatus.textContent=throttle.enabled?'Aktiv':'Nicht aktiv';elements.throttleStatus.className=`status-badge ${throttle.enabled?'status-loaded':'status-error'}`;boxes(elements.throttleCurrent,[['Mindestabstand',seconds(throttle.minimum_interval_ms)],['Letzter reservierter Startslot',throttle.last_reserved_start_ms?dateTime(throttle.last_reserved_start_ms):'—'],['Letzter tatsächlicher BNI-Start',throttle.last_actual_start_ms?dateTime(throttle.last_actual_start_ms):'—'],['Aktuell wartende Anfragen','Nicht zuverlässig messbar']]);boxes(elements.throttleStats,[['Alle gestarteten Anfragen',throttle.requests_24h??0],['Anfragen mit Wartezeit',throttle.waited_requests_24h??0],['Anfragen ohne Wartezeit',throttle.immediate_requests_24h??0],['Noch ohne Wartezeitmessung',Math.max(0,(throttle.requests_24h??0)-(throttle.measured_requests_24h??0))],['Ø Wartezeit',seconds(throttle.avg_wait_ms_24h)],['Max. Wartezeit',seconds(throttle.max_wait_ms_24h)],['p95-Wartezeit',seconds(throttle.p95_wait_ms_24h)]]);const waitPoints=Array.isArray(throttle.series_24h)?throttle.series_24h:[],waitMaximum=Math.max(1,...waitPoints.map(point=>Number(point.maximum_wait_ms)||0)),line=key=>waitPoints.map((point,index)=>`${42+(waitPoints.length<2?0:index/(waitPoints.length-1)*width)},${190-(Number(point[key])||0)/waitMaximum*height}`).join(' ');elements.throttleAverageLine.setAttribute('points',line('average_wait_ms'));elements.throttleMaximumLine.setAttribute('points',line('maximum_wait_ms'));elements.throttleYMax.textContent=seconds(waitMaximum);elements.throttleStart.textContent=waitPoints.length?timeFormat.format(new Date(waitPoints[0].bucket)):'--:--';elements.throttleEnd.textContent=waitPoints.length?timeFormat.format(new Date(waitPoints[waitPoints.length-1].bucket)):'--:--';elements.throttleChart.querySelector('desc').textContent=`Durchschnittliche und maximale Wartezeit in 5-Minuten-Buckets. Maximum ${seconds(waitMaximum)}.`;}
