(() => {
    ...
    function initializeMyAccount() {
        ...
        const renderWatchlist=data=>{if(!watchlistEmail)return;watchlistEmail.checked=data.emailNotifications===true;const links=(data.requests||[]).map(request=>{const link=document.createElement('a');link.href=request.chapterLink||'/?view=crosschaptern';link.textContent=`${request.chapterName} · ${formatDateOnly(request.requestedDate)}`;return link;});if(links.length)watchlistRequests.replaceChildren(...links);else{const empty=document.createElement('span');empty.className='watchlist-empty';empty.textContent='-';watchlistRequests.replaceChildren(empty);}};
