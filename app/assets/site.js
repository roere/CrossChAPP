(() => {
    ...
    const updateWatchlistBadge = data => { const badge=document.querySelector('#watchlist-request-badge');if(!badge)return;const count=Math.max(0,Number(data?.activeRequestChapterCount)||0),text=count===1?'1 beobachtetes Chapter sucht aktuell eine Vertretung.':`${count} beobachtete Chapter suchen aktuell eine Vertretung.`;badge.hidden=count===0;badge.textContent=count===0?'':String(count);badge.title=text;badge.setAttribute('aria-label',text); };
    const refreshWatchlistBadge = async () => { if(!authenticated||!['user','user_manager'].includes(authRole))return null;try{const response=await fetch('/api/auth/watchlist.php'),data=await response.json();if(!response.ok)throw new Error();updateWatchlistBadge(data);return data;}catch{return null;} };
    const initializeWatchlistBadge = () => { const badge=document.querySelector('#watchlist-request-badge');if(!badge)return;badge.addEventListener('click',()=>{const dialog=document.querySelector('#my-account-dialog'),section=document.querySelector('#account-watchlist');if(!dialog?.open)document.querySelector('#open-my-account')?.click();window.setTimeout(()=>{section?.scrollIntoView({block:'nearest'});section?.focus({preventScroll:true});},100);});refreshWatchlistBadge();document.addEventListener('crosschapp:representation-request-changed',refreshWatchlistBadge);document.addEventListener('crosschapp:assignment-changed',refreshWatchlistBadge);};
    ...
    function initializeMyAccount() {
        ...
        const watchlistEmail=document.querySelector('#watchlist-email-notifications'),watchlistMessage=document.querySelector('#watchlist-setting-message'),watchlistRequests=document.querySelector('#watchlist-requests');
