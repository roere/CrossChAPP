...
try {
    ...
    (new RepresentationContactService($database, new RepresentationOfferRepository($database), $settings, new MailService($settings)))->send((int) $identity['user_id'], (int) ($payload['offer_id'] ?? 0), (string) ($payload['requested_date'] ?? ''), (string) ($payload['custom_message'] ?? ''));
