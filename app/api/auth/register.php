...
try { $payload = json_decode(file_get_contents('php://input') ?: '', true, 32, JSON_THROW_ON_ERROR); $result = AccountFactory::create()['service']->register(is_array($payload) ? $payload : [], ClientIp::address()); JsonResponse::send(['registered' => true, 'mailSent' => $result['mailSent'], 'message' => $result['mailSent'] ? 'Bitte bestätige deine E-Mail-Adresse.' : 'Das Konto wurde angelegt. Die Bestätigungs-E-Mail konnte noch nicht versendet werden.']); }
