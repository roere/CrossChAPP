...
final class BniMemberListClient
{
    ...
    public function fetch(int $orgId): array
    {
        ...
        $this->wait();
        $data=['parameters'=>$config['parameters'],'languages'=>$config['languages'],'cmsv3'=>'true','website_type'=>$config['website_type'],'website_id'=>$config['website_id'],'mappedWidgetSettings'=>$config['mapped_widget_settings'],'pageMode'=>'Live_Site'];
        ...
        $http=(int)($result['status']??0);$status=match(true){$http>=200&&$http<300=>'ok',$http===429=>'rate_limited',$http===403=>'forbidden',default=>'upstream_error'};
        return['status'=>$status,'body'=>(string)($result['body']??''),'httpStatus'=>$http];
    }
