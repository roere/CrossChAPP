...
final class RepresentationAssignmentService
{
    ...
    private function assignmentSelect():string{return "SELECT a.*,o.chapter_name chapter,o.chapter_url chapter_url,o.meeting_time meeting_time,r.first_name requester_first_name,r.last_name requester_last_name,r.email requester_email,r.home_chapter_org_id requester_home_chapter_org_id,r.bni_verification_status requester_verification_status,p.first_name representative_first_name,p.last_name representative_last_name,p.email representative_email,p.home_chapter_org_id representative_home_chapter_org_id,p.bni_verification_status representative_verification_status FROM representation_assignments a JOIN organizations o ON o.org_id=a.chapter_org_id JOIN users r ON r.id=a.requester_user_id JOIN users p ON p.id=a.representative_user_id";}
    ...
    private function publicAssignment(array $r,int $userId):array{$requester=$userId===(int)$r['requester_user_id'];$chapterUrl=filter_var((string)($r['chapter_url']??''),FILTER_VALIDATE_URL);if($chapterUrl!==false&&!in_array(strtolower((string)parse_url($chapterUrl,PHP_URL_SCHEME)),['http','https'],true))$chapterUrl=false;return['id'=>(int)$r['id'],'role'=>$requester?'requester':'representative','date'=>(string)$r['representation_date'],'meetingTime'=>(string)($r['meeting_time']??''),'chapter'=>(string)$r['chapter'],'chapterUrl'=>$chapterUrl===false?null:$chapterUrl,'counterpartName'=>(string)($requester?$r['representative_full_name']:$r['requester_full_name']),'counterpartEmail'=>(string)($requester?$r['representative_email']:$r['requester_email']),'counterpartIsBniMember'=>($requester?$r['representative_home_chapter_org_id']:$r['requester_home_chapter_org_id'])!==null,'counterpartVerificationStatus'=>(string)($requester?$r['representative_verification_status']:$r['requester_verification_status']),'status'=>(string)$r['status'],'acceptedAt'=>(string)$r['accepted_at']];}
