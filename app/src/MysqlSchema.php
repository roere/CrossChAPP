...
final class MysqlSchema
{
    ...
    private static function statements(): array
    {
        ...
        return [
            ...
            "CREATE TABLE IF NOT EXISTS automation_runtime(id INT PRIMARY KEY,worker_last_seen_at VARCHAR(32),last_check_at VARCHAR(32),next_check_at VARCHAR(32),last_map_refresh_at VARCHAR(32),map_lock_token VARCHAR(128),map_lock_until VARCHAR(32),map_retry_after_until VARCHAR(32),last_watchlist_notification_check_at VARCHAR(32),updated_at VARCHAR(32) NOT NULL)$engine",
            ...
            "CREATE TABLE IF NOT EXISTS user_chapter_watchlist(id BIGINT AUTO_INCREMENT PRIMARY KEY,user_id BIGINT NOT NULL,organization_id BIGINT NOT NULL,created_at VARCHAR(32) NOT NULL,UNIQUE KEY uq_watchlist_user_organization(user_id,organization_id),INDEX idx_watchlist_user(user_id),INDEX idx_watchlist_organization(organization_id),FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE,FOREIGN KEY(organization_id) REFERENCES organizations(org_id) ON DELETE CASCADE)$engine",
            "CREATE TABLE IF NOT EXISTS watchlist_notifications(id BIGINT AUTO_INCREMENT PRIMARY KEY,user_id BIGINT NOT NULL,representation_request_id BIGINT NOT NULL,channel VARCHAR(20) NOT NULL,sent_at VARCHAR(32) NOT NULL,created_at VARCHAR(32) NOT NULL,UNIQUE KEY uq_watchlist_notification(user_id,representation_request_id,channel),INDEX idx_watchlist_notifications_user(user_id),INDEX idx_watchlist_notifications_request(representation_request_id),FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE,FOREIGN KEY(representation_request_id) REFERENCES representation_requests(id) ON DELETE CASCADE)$engine",
